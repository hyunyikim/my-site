  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-screen max-w-screen-xl m-auto px-4 md:px-8 lg:px-20 py-8 lg:py-0 lg:-right-8">
      <header className="flex flex-col h-max lg:sticky lg:top-0 lg:py-24 reveal d-0">
        ...
        <button type="button" onClick={playNameSound} className="listen-pill self-start mb-6" title="Listen to pronunciation">
          <AiFillSound size={11} />
          <span>hear it</span>
        </button>
        ...
        <main className="flex flex-col gap-8 lg:gap-16 lg:pt-16 lg:pb-24">
          ...
          <section className="reveal d-2">
            ...
            <div className="flex flex-row items-baseline justify-between flex-wrap gap-3">
              <h3 className="display text-3xl sm:text-4xl font-light">Working Experience</h3>
              <a className="flex flex-row gap-1.5 items-center group text-sm tracking-wide" href="https://assets.hyunyikim.xyz/public/CV.pdf" target="_blank">
                ...
                <FiExternalLink size={13} className="transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
              </a>
            </div>
            <ul className="flex flex-col gap-3 mt-4">
              {WORK_EXPERIENCES.map((exp) => (
                <li key={`${exp.company}-${exp.period}`} className="project-card no-link">
                  ...
                  {exp.description && <div className="text-sm opacity-75 leading-relaxed">{exp.description}</div>}
                </li>
              ))}
            </ul>
          </section>
          <section className="reveal d-3">
            ...
            <h3 className="display text-3xl sm:text-4xl font-light mb-4">Side Projects</h3>
            <ul className="flex flex-col gap-3">
              {SIDE_PROJECTS.map((project) => (
                <li
                  ...
                  <div className={`flex flex-row flex-wrap items-center justify-between gap-2${project.icon ? ' pr-6' : ''}`}>
                    <div className="flex flex-row items-center gap-2.5">
                      {project.icon && <Image src={project.icon} alt={project.name} width={28} height={28} className="rounded" />}
                      <span className="font-semibold text-base">{project.name}</span>
                    </div>
                    <div className="flex flex-row gap-1.5">
                      <span className="tag-chip">{project.tag}</span>
                      {project.inDevelopment && <span className="tag-chip warn">In development</span>}
                    </div>
                  </div>
                  <div className="text-sm opacity-75 leading-relaxed">{project.description}</div>
                  {project.techStack && project.techStack.length > 0 && (
                    <div className="flex flex-row flex-wrap gap-1.5 mt-2">
                      {project.techStack.map((tech) => (
                        <span key={tech} className="text-xs px-2 py-0.5 rounded-full border border-current opacity-50">
                          {tech}
                        </span>
                      ))}
                    </div>
          ...
          <section className="reveal d-4">
            ...
            <h3 className="display text-3xl sm:text-4xl font-light mb-5">What I like</h3>
            <article className="grid grid-cols-2 grid-rows-4 gap-2">
              <div className="favoriate vertical" data-caption="Berliner Dom">
                <Image src="/images/IMG_5864.jpg" alt="berlin dom" fill className="object-cover hover:scale-105 transition-transform duration-500" />
              </div>
              <div className="favoriate vertical" data-caption="A single leaf">
                <Image src="/images/IMG_6081.jpg" alt="leaf" fill className="object-cover hover:scale-105 transition-transform duration-500" />
              </div>
              <div className="favoriate horizontal col-span-2" data-caption="Sunset over Tempelhof">
                <Image src="/images/IMG_7279.jpg" alt="sunset in tempelhof" fill className="object-cover hover:scale-105 transition-transform duration-500" />
              </div>
              <div className="favoriate vertical" data-caption="A very good dog">
                <Image src="/images/IMG_6646.jpg" alt="dog" fill className="object-cover hover:scale-105 transition-transform duration-500" />
              </div>
              <div className="favoriate vertical" data-caption="Ice cream afternoons">
                <Image src="/images/IMG_3241.jpg" alt="icecream" fill className="object-cover hover:scale-105 transition-transform duration-500" />
              </div>
              <div className="favoriate vertical" data-caption="Backlit leaves">
                <Image src="/images/IMG_6846.jpg" alt="leaf" fill className="object-cover hover:scale-105 transition-transform duration-500" />
              </div>
              <div className="favoriate vertical" data-caption="Ein Bier, bitte">
                <Image src="/images/IMG_7424.jpg" alt="beer" fill className="object-cover hover:scale-105 transition-transform duration-500" />
              </div>
              <div className="favoriate horizontal col-span-2" data-caption="Roses in bloom">
                <Image
